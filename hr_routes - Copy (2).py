...
def get_attendance():
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT Pk_AttendanceID, Fk_EmployeeID, PunchTime, Status, ApprovedStatus, PunchInTime, PunchOutTime, Location, AdminName, AdminNote FROM tblAttendance")
        rows = cursor.fetchall()
...
def update_setting():
    ...
    try:
        data = request.get_json() or {}
        key = data.get('key')
        value = data.get('value')  # Yeh 1 ya 0 hoga
        ...
        return jsonify({"success": True, "message": f"Setting updated for CLID {clid} successfully!"}), 200
    except Exception as e:
        if conn: conn.close()
        return jsonify({"success": False, "error": str(e)}), 500
...
def get_client_branches():
    ...
    assigned_biz = session.get('assigned_businesses', '')  # Jaise "Alokk, Nirwana"
